<#
.SYNOPSIS
	Will set a tatoo information into an OS image during OSD deployment.
...
.NOTES
	...
	-Version: 1.4.2
    -History:
    1.4.2 ; 04.14.2015 ; Changed variable name: SCCMOSD_TaskSequenceInstallationName --> SCCMOSD_TSName
    1.4.2 ; 04.14.2015 ; Changed variable name: SCCMOSD_TaskSequenceInstallationID --> SCCMOSD_TSID  
...
"Starting operations" >> $LogFile
    ...
    $SCCMOSD_TattooScriptVersion = "1.4.2"

 "Script version is $($SCCMOSD_TattooScriptVersion)">> $LogFile    

    $SCCMOSD_TSName = $tsenv.Value("_SMSTSPackageName")
    $BootImageVersion = $tsenv.Value("_SMSTSBootImageID")
    $SCCMOSD_DeploymentID = $tsenv.Value("_SMSTSPackageID")
    $SCCMOSD_InstallationMethod = $tsenv.Value("_SMSTSMediaType")
    $SCCMOSD_TSID = $tsenv.Value("_SMSTSPackageID")
    $SCCMOSD_SiteCode = $tsenv.Value("_SMSTSSiteCode")
    $SCCMOSD_OsBuildversion = $tsenv.Value("OsBuildVersion")

    
    $SCCMOSD_InstallationDate = get-date -uformat "%Y%m%d-%T"

    $CustomVariables = @()
    $CustomVariables = $tsenv.getVariables() | ? {$_ -match "SCCMOSD_*"}
    ...
    $VarTatoos = Get-Variable -Name "SCCMOSD_*"
    ...
    if ($PSBoundParameters.ContainsKey("WMI") -or $PSBoundParameters.ContainsKey("All")){
        #Creating WMI instance
                ...
                Set-WMIPropertyQualifier -className $Root -PropertyName "SCCMOSD_TattooScriptVersion" -key
